...
- (NSUInteger)readLengthForTerm
{
	...
	while (i < bytesDone)
	{
		void *subbuf = (char *)buf + startOffset + i;
...
- (NSInteger)searchForTermAfterPreBuffering:(NSUInteger)numBytes
{
	...
	{
		void *subBuffer = (char *)[buffer mutableBytes] + startOffset + i;
...
- (void)recoverUnreadData
{
	if(theCurrentRead != nil)
	{
		...
		if(([theCurrentRead isKindOfClass:[AsyncReadPacket class]]) && (theCurrentRead->bytesDone > 0))
		{
			...
			void *buffer = (char *)[theCurrentRead->buffer mutableBytes] + theCurrentRead->startOffset;
...
- (NSData *)unreadData
{
	...
	while (!error && CFReadStreamHasBytesAvailable(theReadStream))
	{
		...
		UInt8 *packetbuf = (UInt8 *)((char *)[partialReadBuffer mutableBytes] + totalBytesRead );
...
- (void)doBytesAvailable
{
	...
	while(!done && !socketError && !maxoutError && [self hasBytesAvailable])
	{
		...
		void *buffer = (char *)[theCurrentRead->buffer mutableBytes] + theCurrentRead->startOffset;
		void *subBuffer = (char *)buffer + theCurrentRead->bytesDone;
			...
			else if (theCurrentRead->term != nil)
			{
				...
				if (didPreBuffer)
				{
					...
					if (overflow > 0)
					{
						...
						void *overflowBuffer = (char *)buffer + theCurrentRead->bytesDone - overflow;
				...
				else
				{
					...
					{
						void *bufferEnd = (char *)buffer + (theCurrentRead->bytesDone - termlen);
...
- (void)doSendBytes
{
	...
	while (!done && !error && [self canAcceptBytes])
	{
		...
		UInt8 *writestart = (UInt8 *)((char *)[theCurrentWrite->buffer bytes] + theCurrentWrite->bytesDone);
